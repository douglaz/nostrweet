...
- **pre-commit**: Runs `cargo fmt --all -- --check` to ensure code is formatted
- **pre-push**: Runs both `cargo fmt --all -- --check` and `cargo clippy --all-targets --all-features -- -D warnings`
